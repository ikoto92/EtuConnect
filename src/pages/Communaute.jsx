...
import PageCommunaute from "./pageCommunaute";
...
function Communaute() {
  const [users, setUsers] = useState(() => [
    ...
    },
    {
      ...
      posts: [
        {
          typePhoto: "photo",
          content: "https://i.etsystatic.com/30101587/r/il/c048ef/3801953885/il_fullxfull.3801953885_tqnn.jpg",
          date: new Date().toLocaleString(),
        },
      ],
    },
      ...
      prevUsers.map((user) =>
        user.id === userId
          ? {
              ...
              posts: [
                ...user.posts,
                { id: Date.now(), ...newPost, date: new Date().toLocaleString() },
              ],
            }
          : user
      )
  ...
  return (
    <div>
      ...
      {users.map((user) => (
        <PageCommunaute key={user.id} user={user} onAddPost={addPost} />
      ))}
    </div>
  );
}
